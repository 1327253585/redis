...
static pthread_t bio_threads[BIO_WORKER_NUM];  // 工作线程
static pthread_mutex_t bio_mutex[BIO_WORKER_NUM]; // 锁
static pthread_cond_t bio_newjob_cond[BIO_WORKER_NUM]; //    条件变量
static list *bio_jobs[BIO_WORKER_NUM]; // 工作队列
static unsigned long bio_jobs_counter[BIO_NUM_OPS] = {0}; // 记录每个job的数量
...
static list *bio_comp_list; //  完成的job响应队列
...
typedef union bio_job {
    ...
    struct {
        ...
        long long offset; /* A job-specific offset, if applicable */   // offset  暂时用不到
        unsigned need_fsync:1; /* A flag to indicate that a fsync is required before  
                                * the file is closed. */  // 关闭前是否需要刷盘
...
void bioInit(void) {
    ...
    // 每个后台都有一个锁和条件变量, 用于保护当前任务队列以及唤醒任务线程
    ...
    pthread_mutex_init(&bio_mutex_comp, NULL); // comp 线程, 用于通知
    ...
    if (anetPipe(job_comp_pipe, O_CLOEXEC|O_NONBLOCK, O_CLOEXEC|O_NONBLOCK) == -1) { // 创建一个管道
    ...
    if (aeCreateFileEvent(server.el, job_comp_pipe[0], AE_READABLE,
                          bioPipeReadJobCompList, NULL) == AE_ERR) {  // 注册一个读事件, 用于唤醒主线程
     ...
     * responsible for. */
    for (j = 0; j < BIO_WORKER_NUM; j++) { // 创建3个work线程
...
*    @job 任务
*    @return null
* 每个任务都有一个任队列,加锁把当前的任务放到任务队列里, 同时唤醒对应的工作线程
*/
...
// 提交free大对象的任务队列, 释放的函数, free的其他参数列表
...
void *bioProcessBackgroundJobs(void *arg) {
    ...
    pthread_mutex_lock(&bio_mutex[worker]);           // 加锁
    ...
    while(1) {
        ...
        if (listLength(bio_jobs[worker]) == 0) {   // 当前任务对应的任务队列为空
            pthread_cond_wait(&bio_newjob_cond[worker], &bio_mutex[worker]);  // 加锁进行等待 
            continue;
        }
        /* Get the job from the queue. */
        ln = listFirst(bio_jobs[worker]); // 从队列中取出一个任务
        job = ln->value;
        /* It is now possible to unlock the background system as we know have
         * a stand alone job structure to process.*/
        pthread_mutex_unlock(&bio_mutex[worker]);  // 可以释放锁了

        /* Process the job accordingly to its type. */
        int job_type = job->header.type; 

        if (job_type == BIO_CLOSE_FILE) { // 根据任务类型调用关闭close函数
            ...
            if (job_type == BIO_CLOSE_AOF)
                close(job->fd_args.fd);
        } else if (job_type == BIO_LAZY_FREE) {         // 释放大对象, 调用对应的释放函数
            job->free_args.free_fn(job->free_args.free_args);
        } else if ((job_type == BIO_COMP_RQ_CLOSE_FILE) ||      // 其他的任务类型
            ...
            if (write(job_comp_pipe[1],"A",1) != 1) {   // 往管道里写入数据
        ...
        pthread_cond_signal(&bio_newjob_cond[worker]);   // 发送信号
    }
    ...
    //  - 队列为空, 则 等待唤醒
    //  - 不为空, 
    //  则 取出任务, 释放锁, 处理任务, 加锁, 处理完任务后, 发送信号, 继续循环 
}
...
void bioPipeReadJobCompList(aeEventLoop *el, int fd, void *privdata, int mask) {
    ...
    while (listLength(tmp_list)) {  // 通知所有的任务完成的回调函数, 通知完成后删除
